...
print(strat.num_replicas_in_sync)
input()
...
with strat.scope():
	...
	def dist_train_step(batch):
		replica_loss_output = strat.experimental_run_v2(train_step, args=(batch,))
		return strat.reduce(tf.distribute.ReduceOp.SUM, replica_loss_output, axis=None)

	@tf.function
	def dist_val_step(batch):
		replica_loss_output = strat.experimental_run_v2(val_step, args=(batch,))
		return strat.reduce(tf.distribute.ReduceOp.SUM, replica_loss_output, axis=None)
	...
	for epoch in range(EPOCHS):
		...
		for batch in train_set_batch:
			loss_output = dist_train_step(batch)
		...
		for batch in val_set_batch:
			 val_loss.append(dist_val_step(batch).numpy())
